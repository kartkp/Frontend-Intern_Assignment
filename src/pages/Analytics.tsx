...
function AnalyticsPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-blue-200 via-purple-200 to-pink-200 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
  ...
  if (error || !analyticsData) {
    return (
      <div className="flex items-center justify-center h-screen bg-red-50 dark:bg-gray-900">
        <div className="bg-white/30 dark:bg-gray-800/30 p-6 rounded-3xl shadow-2xl backdrop-blur-md text-center space-y-2 border border-red-400">
          <p className="text-red-600 dark:text-red-400 text-lg font-semibold">Error Loading Analytics</p>
  ...
  return (
    <div className="relative">
      <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
        <h1 className="text-4xl md:text-5xl font-bold mb-10 text-center text-gray-900 dark:text-white drop-shadow-xl">
          Analytics Dashboard
        </h1>
    ...
    <GradientCard title="Top Performers">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {performers.map(user => (
          <div
            key={user.id}
            className="flex items-center space-x-4 bg-white/60 dark:bg-gray-700/40 rounded-xl p-3 shadow hover:shadow-lg transition"
          >
            <img
            ...
            <div>
              ...
              <div className="flex items-center space-x-0.5 mt-1">
                {Array.from({ length: user.performance }).map((_, idx) => (
                  <span key={idx} className="text-yellow-400">★</span>
                ))}
                {Array.from({ length: 5 - user.performance }).map((_, idx) => (
                  <span key={idx} className="text-gray-400 dark:text-gray-600">★</span>
                ))}
              </div>
            </div>
          </div>
...
function GradientCard({ title, children }: GradientCardProps) {
  return (
    <div className="rounded-3xl bg-white/10 dark:bg-gray-800/20 backdrop-blur-lg border border-white/20 dark:border-gray-700 shadow-xl p-6 hover:shadow-2xl transition-all duration-300">
      <h2 className="text-2xl font-semibold mb-4 text-gray-800 dark:text-gray-100 drop-shadow">{title}</h2>
      {children}
    </div>
  )
}
...
function SummaryCard({ title, value, color }: SummaryCardProps) {
  ...
  return (
    <div className={`rounded-2xl p-4 bg-gradient-to-br ${gradients[color]} text-white shadow-lg transition-transform duration-300 hover:scale-105`}>
